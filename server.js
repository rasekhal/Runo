...
function onMessage(client, message) {
  ...
  if (type === 'drawCard') { if (!isTurn(room, client)) return; const player = room.players.find((entry) => entry.client === client); const hadPenalty = room.pendingDraw > 0; const drawCount = Math.max(1, room.pendingDraw); for (let index = 0; index < drawCount; index += 1) { if (!room.deck.length) room.deck = makeDeck(); player.hand.push(room.deck.pop()) } room.pendingDraw = 0; player.unoCalled = false; if (room.lastUnoCall?.playerId === player.id) room.lastUnoCall = null; advanceAfterDraw(room, hadPenalty); broadcast(room); return }
