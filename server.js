...
function join(client, room, name) { const player = { id: client.id, name: String(name || 'Player').slice(0, 18), client, hand: [] }; room.players.push(player); clients.set(client, { room }); broadcast(room) }
function isTurn(room, client) { return room.status === 'playing' && room.players[room.turn]?.client === client }
function playCard(room, client, index, chosenColor) { if (!isTurn(room, client)) return; const player = room.players.find((entry) => entry.client === client); const card = player.hand[index]; const isPenaltyCard = card?.value === '+2' || card?.value === '+4'; const canStack = room.settings.cardStacking && room.pendingDraw > 0 && isPenaltyCard; const canMatch = room.pendingDraw === 0 && (card?.color === 'wild' || card?.color === room.activeColor || card?.value === room.topCard.value); if (!card || (!canStack && !canMatch)) return; if (card.color === 'wild' && !colors.includes(chosenColor)) return; player.hand.splice(index, 1); room.topCard = { ...card }; room.activeColor = card.color === 'wild' ? chosenColor : card.color; if (room.settings.cardStacking && isPenaltyCard) room.pendingDraw += card.value === '+4' ? 4 : 2; if (!player.hand.length) { room.status = 'waiting'; room.round += 1; room.players.forEach((entry) => { entry.hand = [] }); broadcast(room); return } advanceAfterCard(room, card, isPenaltyCard); broadcast(room) }
